...
// bool getDecision(Instruction* instruction) {
// 	assert(instruction->type != BLOCK && "A block is an invalid instruction for decision");
// 	assert(tk_in_type_arr(instruction->tk, decisionNeeded, ARR_SZ(decisionNeeded)) && "Invalid instruction");
// 	TOKEN_TYPE tk_t = instruction->tk->type;
// 	if (tk_t == HEAD_READ) {
// 		return readByteAtPosition(*selectedByte, x_pos);
// 	}
// 	else if (tk_t == CHECK_ABILITY_TO_MOVE) {
// 		assert(false && "Not implemented");
// 		return false;
// 	}
// 	else if (tk_t == LOOP_UNTIL_END)       { return x_pos == 8; }
// 	else if (tk_t == LOOP_UNTIL_BEGINNING) { return x_pos == -1; }
// 	else if (tk_t == LOOP_FIXED_AMOUNT)    {}
// 	else if (tk_t == LOOP_DYNAMIC_AMOUNT)  {}
// 	else {
...
// 	Instruction* instruction_ptr = prog->instruction_tree;
// 	Instruction* end_of_program = prog->instruction_tree->block_context->junior;

// 	while (instruction_ptr != end_of_program) {
// 		if (instruction_ptr->type == BLOCK) {
// 			instruction_ptr = instruction_ptr->next_unconditional;
// 		}
// 		else if (tk_in_type_arr(instruction_ptr->tk, decisionNeeded, ARR_SZ(decisionNeeded))) {
// 			bool decision = false;
// 			//do specific
// 			if (decision) instruction_ptr = instruction_ptr->next_if_true;
// 			else          instruction_ptr = instruction_ptr->next_if_false;
// 		}
// 		else {
// 			//do specific
// 			instruction_ptr = instruction_ptr->next_unconditional;
...
// void Railcar_Simulator_old(Program* prog) {
